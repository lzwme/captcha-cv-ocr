...
class TesseractOcr {
  ...
  constructor(workerConfigList = [{ ...defaultWorkerConfig }], autoClose = false) {
  ...
  async init(uid) {
    for (const [id, { config, scheduler }] of Object.entries(this.#schedulerCache)) {
      ...
      for (let i = 0; i < config.num; i++) {
        const worker = await createWorker(config.langs || ['eng'], config.oem || OEM.DEFAULT, {
          ...config.options,
  ...
  autoTerminate() {
    ...
    this.#autoCloseTimer = setTimeout(() => this.getQueueLen() === 0 ? this.terminate() : this.autoTerminate(), 20_000);
  }
